 ...
 */
export async function transcribeAudio(audioUrl, bearerToken) {
 ...
 */
export async function resolveMetaMediaUrl(mediaId) {
    ...
    try {
        const res = await fetch(`https://graph.facebook.com/${WHATSAPP_API_VERSION}/${mediaId}`, { headers: { Authorization: `Bearer ${token}` } });
        if (!res.ok) {
