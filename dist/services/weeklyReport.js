...
export async function sendAllWeeklyReports() {
    ...
    for (const business of businesses) {
        try {
            ...
            const caEstime = orders.reduce((sum, o) => sum + (o.price ?? 0) * (o.quantity ?? 1) + (o.deliveryPrice ?? 0), 0);
            const confirmationRate = totalOrders > 0 ? Math.round(((confirmedOrders + deliveredOrders) / totalOrders) * 100) : 0;
            const returnRate = totalOrders > 0 ? Math.round((cancelledOrders / totalOrders) * 100) : 0;
            ...
            const cityCounts = orders.reduce((acc, o) => {
                if (o.address)
                    acc[o.address] = (acc[o.address] ?? 0) + 1;
                return acc;
            }, {});
