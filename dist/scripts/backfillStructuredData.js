...
async function processOrder(order) {
    try {
        ...
        if (fields.deliveryCompany)
            data.deliveryCompany = fields.deliveryCompany;
...
async function main() {
    ...
    const uniqueCustomers = [
        ...new Map(orders.map((o) => [`${o.customerPhone}|${o.businessId}`, o])).values(),
    ];
    ...
    for (let i = 0; i < uniqueCustomers.length; i += CONCURRENCY) {
        ...
        await Promise.allSettled(sub.map(async (c) => {
            try {
...
//# sourceMappingURL=backfillStructuredData.js.map