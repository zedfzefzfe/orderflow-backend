...
async function sendPushNotification(businessId, order) {
    try {
...
function detectRole(senderPhone, business) {
...
async function saveConversationMessage(businessId, phone, role, type, content, mediaUrl, processed = false) {
...
async function resolveCustomerPhone(businessId, senderPhone, role) {
    if (role === 'client')
        return senderPhone;
...
async function markConversationProcessed(businessId, phone) {
...
async function sendWhatsAppMessage(to, message, _businessId) {
...
async function sendWhatsAppMessageToMerchant(text, businessId) {
    const business = await prisma.business.findUnique({ where: { id: businessId } });
    if (!business?.ownerNotifyPhone) {
...
function parseTriggerDetails(lines) {
    ...
    const productLine = details.find(l => !/\d+\s*dh/i.test(l) &&
        !/livraison/i.test(l) &&
        !/prix/i.test(l) &&
        l.trim().length > 0) || null;
...
async function handleTrigger(business, customerPhone, messageText) {
    ...
    if (!productInfo.product) {
        ...
        await sendWhatsAppMessageToMerchant(`⚠️ Format incorrect. Envoyez :\n"commande confirmée\n[nom du produit]\n[prix]dh\nlivraison [prix livraison]dh"\n\nExemple :\ncommande confirmée\nbouquet rose rouge\n299dh\nlivraison 25dh`, businessId);
        return;
    }
    ...
    const PLAN_LIMITS = { trial: 20, starter: 200, growth: -1, pro: -1 };
...
async function handleTwilioPayload(req, res) {
    ...
    if (isTwilioTrigger) {
        ...
        handleTrigger(business, customerPhone, content).catch((err) => console.error('[webhook/twilio] handleTrigger error:', err));
    }
    res.type('text/xml').send('<?xml version="1.0" encoding="UTF-8"?><Response></Response>');
}
// ── Meta handler ───────────────────────────────────────────────────────────────
        ...
        for (const change of changes) {
            ...
            for (const message of messages) {
                ...
                if (isMetaTrigger) {
                    ...
                    handleTrigger(business, customerPhone, content).catch((err) => console.error('[webhook/meta] handleTrigger error:', err));
                }
            }
        }
    }
...
async function handleYCloudPayload(body) {
    ...
    if (msgType === 'text') {
        ...
        mediaUrl = (audioObj?.url ?? audioObj?.link ?? audioObj?.cdnUrl ?? audioObj?.downloadUrl);
        const mimeHint = (audioObj?.mimeType ?? audioObj?.mime_type ?? '');
    ...
    if (role === 'client' && type === 'text') {
        ...
        if (pendingOrder) {
            ...
            if (!hasInfo) {
                ...
                const updateData = {};
                if (result.name && !pendingOrder.customerName)
                    updateData.customerName = result.name;
                if (result.address && !pendingOrder.address)
                    updateData.address = result.address;
                if (result.date && !pendingOrder.deliveryDate)
                    updateData.deliveryDate = result.date;
                if (result.phone && !pendingOrder.phone)
...
شكراً على ثقتكم !`;
    ...
    if (isYCloudTrigger) {
        ...
        handleTrigger(business, customerPhone, content).catch((err) => console.error('[webhook/ycloud] handleTrigger error:', err));
    }
}
...
async function handleYCloudEcho(body) {
    ...
    const wabaId = (message.wabaId ?? body.wabaId);
    ...
    if (msgType === 'text') {
        ...
        const audioUrl = (audioObj?.url ?? audioObj?.link ?? audioObj?.cdnUrl ?? audioObj?.downloadUrl);
    ...
    const echoType = msgType === 'audio' || msgType === 'voice' ? 'audio' : msgType === 'image' ? 'image' : 'text';
    ...
    if (isEchoTrigger) {
        ...
        handleTrigger(business, resolvedCustomerPhone, content).catch((err) => console.error('[webhook/ycloud/echo] handleTrigger error:', err));
    }
}
