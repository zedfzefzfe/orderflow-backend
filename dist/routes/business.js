...
router.patch('/me', requireAuth, async (req, res) => {
    try {
        ...
        if (ownerNotifyPhone !== undefined)
            data.ownerNotifyPhone = ownerNotifyPhone ? String(ownerNotifyPhone).trim() : null;
        if (confirmationTemplate !== undefined)
            data.confirmationTemplate = confirmationTemplate ? String(confirmationTemplate) : null;
        if (formulaireTemplate !== undefined)
            data.formulaireTemplate = formulaireTemplate ? String(formulaireTemplate) : null;
        if (email !== undefined)
            data.email = email ? String(email).trim().toLowerCase() : null;
        if (weeklyReportEnabled !== undefined)
            data.weeklyReportEnabled = Boolean(weeklyReportEnabled);
        if (sector !== undefined)
            data.sector = sector || null;
        if (defaultDeliveryCompany !== undefined)
            data.defaultDeliveryCompany = defaultDeliveryCompany || null;
...
router.post('/test-weekly-report', requireAuth, async (req, res) => {
    try {
        const business = await prisma.business.findUnique({
            where: { id: req.user.businessId },
        });
