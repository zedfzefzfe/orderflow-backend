...
router.get('/client/:phone', requireAuth, async (req, res) => {
    try {
        ...
        const totalCA = orders.reduce((sum, o) => sum + (o.price ?? 0) * (o.quantity ?? 1) + (o.deliveryPrice ?? 0), 0);
...
router.patch('/:id', requireAuth, async (req, res) => {
    try {
        ...
        if (customerName !== undefined)
            data.customerName = String(customerName).trim();
        if (customerPhone !== undefined)
            data.customerPhone = String(customerPhone).trim();
        if (product !== undefined)
            data.product = String(product).trim();
        if (address !== undefined)
            data.address = address === '' ? null : String(address).trim();
        if (deliveryDate !== undefined)
            data.deliveryDate = deliveryDate === '' ? null : String(deliveryDate).trim();
        ...
        if (price !== undefined) {
            if (price === null) {
                ...
                const effectiveQty = data.quantity ?? order.quantity;
                data.price = priceNum;
            ...
            data.totalPrice = order.price * data.quantity;
        }
        if (deliveryPrice !== undefined) {
            if (deliveryPrice === null) {
                ...
                if (isNaN(dpNum) || dpNum < 0) {
                    res.status(400).json({ error: 'Invalid deliveryPrice' });
                    return;
                }
...
router.post('/', requireAuth, async (req, res) => {
    try {
        ...
        if (!customerName || !product) {
            res.status(400).json({ error: 'customerName and product are required' });
            return;
        }
