...
async function buildContext(businessId) {
    const now = new Date();
    const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const twoHoursAgo = new Date(now.getTime() - 7_200_000);
    const [business, todayOrders, thisMonthOrders, topProducts, topClients, topCities, recentOrders, catalog, pendingOrders,] = await Promise.all([
    ...
    const confirmedThisMonth = thisMonthOrders.filter(o => o.status === 'CONFIRMED' || o.status === 'DELIVERED').length;
    return {
        ...
        thisMonth: {
            ...
            confirmationRate: thisMonthOrders.length > 0
