...
async function main() {
  ...
  const pairs = await prisma.order.groupBy({
    ...
    where: {
      status: { in: ['LIVRE', 'DELIVERED', 'RETOURNE', 'ANNULE', 'CANCELLED'] as any[] },
    },
  });
  ...
  const deduped: { customerPhone: string; businessId: string }[] = [];
  ...
  for (const { customerPhone, businessId } of deduped) {
    try {
      ...
      ok++;
      if (ok % 50 === 0) console.log(`[backfill]   ${ok}/${deduped.length}...`);
    } catch (err) {
      errors++;
