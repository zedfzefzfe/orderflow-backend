...
router.get('/status-breakdown', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const businessId = req.user!.businessId;
    const days = parseInt(req.query.period as string) || 30;
...
router.get('/revenue-by-day', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const businessId = req.user!.businessId;
    const days = parseInt(req.query.period as string) || 30;
    const { start } = getPeriodRange(days);
...
router.get('/orders-comparison', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const businessId = req.user!.businessId;
    const now = new Date();
    ...
    for (const o of thisMonthOrders) thisMonthByDay[o.createdAt.getDate()] = (thisMonthByDay[o.createdAt.getDate()] || 0) + 1;

    const lastMonthByDay: Record<number, number> = {};
    for (const o of lastMonthOrders) lastMonthByDay[o.createdAt.getDate()] = (lastMonthByDay[o.createdAt.getDate()] || 0) + 1;
...
router.get('/weekly-rates', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const businessId = req.user!.businessId;
    const now = new Date();
    ...
    const buckets: { total: number; confirmed: number }[] = Array.from({ length: 8 }, () => ({ total: 0, confirmed: 0 }));
    ...
    for (const o of orders) {
      ...
      if (o.status === 'CONFIRMED' || o.status === 'DELIVERED') buckets[bucket].confirmed++;
    }
...
router.get('/top-cities', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    res.json(rows.filter((r) => r.address).map((r) => ({ city: r.address as string, count: r._count.address })));
  } catch (err) {
